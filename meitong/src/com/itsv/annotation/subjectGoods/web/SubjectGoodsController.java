...
/**
 * 说明：增加，修改，删除subject_goods的前端处理类
 * 
 * @author swk
 * @since 2016-03-25
 * @version 1.0
...
public class SubjectGoodsController extends BaseAnnotationController<SubjectGoods> {
	...
	protected void beforeShowEdit(HttpServletRequest request, HttpServletResponse response, ModelAndView mnv) {
		...
		if (null == subjectGoods) {
			showMessage(request, "未找到对应的subject_goods记录。请重试");
	...
	public ModelAndView saveAdd(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			showMessage(request, "新增subject_goods成功");
		} catch (AppException e) {
			logger.error("新增subject_goods[" + subjectGoods + "]失败", e);
			showMessage(request, "新增subject_goods失败：" + e.getMessage(), e);

			//增加失败后，应将已填写的内容重新显示给subject_goods
			return new ModelAndView(getAddView(), WebConfig.DATA_NAME, subjectGoods);
		}
	...
	public ModelAndView saveEdit(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			showMessage(request, "修改subject_goods成功");
		} catch (AppException e) {
			logger.error("修改subject_goods[" + subjectGoods + "]失败", e);
			showMessage(request, "修改subject_goods失败：" + e.getMessage(), e);
	...
	public ModelAndView delete(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			showMessage(request, "删除subject_goods成功");
		} catch (AppException e) {
			logger.error("批量删除subject_goods时失败", e);
			showMessage(request, "删除subject_goods失败：" + e.getMessage(), e);
		}
